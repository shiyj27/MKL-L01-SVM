function  out= MKL01ADMM_d(x,y,pars)
...
% if isfield(pars,'numRuns');  sig = pars.sig; else; sig = 1;    end
...
if pars.numRuns==2
figure;
plot(FirstDvalues, 'b','LineWidth',1); % 'b-o'代表蓝色线条带圆圈标记
hold on ;
plot(SecondDvalues, 'r','LineWidth',1); % 'b-o'代表蓝色线条带圆圈标记
% hold on;
plot(ThirdDvalues, 'g','LineWidth',1); % 'b-o'代表蓝色线条带圆圈标记
xlabel('Iteration');
ylabel('$\mathrm{d_k} \quad MKL-L_{0/1}-SVM$', 'Interpreter', 'latex');
% legend('d_1', 'd_{4}', 'd_{10}') %wpbc

grid on;
% 设置坐标轴标签和图例的字体大小
set(gca, 'FontSize', 12); % 12为你想要的字体大小
% lgd = legend('d_6', 'd_{3}', 'd_{5}'); %iono
lgd = legend('d_1', 'd_{4}', 'd_{10}') ;%wpbc
set(lgd, 'FontSize', 12);

% 如果想要设置标题的字体大小
titleFontSize = 16;
% title('Ionosphere', 'FontSize', titleFontSize);
title('Wpbc', 'FontSize', titleFontSize);
hold off
end
